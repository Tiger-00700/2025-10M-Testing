#!/usr/bin/env python3
"""
Book builder script - combines framework files into a single book
"""
...
def build_book():
    ...
    book_content = []
    book_content.append("<!-- 自动生成：已按 /framework/ 目录顺序插入 第1篇～第7篇 的完整内容（{}）。来源：/framework/*.md -->\n".format(datetime.now().strftime("%Y-%m-%d")))
    ...
    # Add final note
    book_content.append(f"\n<!-- 自动化注记：已按 /framework/ 目录顺序插入第1篇～第7篇（{datetime.now().strftime('%Y-%m-%d')}）。下一步：运行验证脚本检查残留占位标记（如\"12.16建议\"）、移除任意\"truncated\"占位并修正重复段落，以及统计行数对比与生成 QA 报告。 -->\n")
