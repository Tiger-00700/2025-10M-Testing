#!/usr/bin/env python3
"""
Auto-fill learning blocks (学习目标 / 小结 / 练习) for the entire book with selectable style.
...
CN_STOPS = set('''
本节 本章 本篇 本书 本文 章节 小结 练习 学习 目标 介绍 概述 定义 内容 与 及 和 或 以及 持续 性能 安全 数据 系统 方法 技术 工具 应用 案例 场景 流程 模型 架构 设计 分析 测试 大数据 平台 组件 相关 常见 问题 说明 参考 示例 实践 最佳 框架 质量 指标 管理 方案 策略 结构 目的 范围 背景
'''.split())
...
def tokenize_cn(text: str) -> list[str]:
	# very light tokenizer: split by punct/space; keep token length>=2; filter digits-only
...
def top_keywords(title: str, body_lines: list[str], k: int = 5) -> list[str]:
	...
	sample = [title]
	bullets = [l for l in body_lines[:80] if l.strip().startswith(('-', '*'))]
...
def gen_learning_goals(title: str, kw: list[str], style: str) -> list[str]:
	...
	bullets = [
		f"理解“{title}”的关键概念与边界，能用自己的话归纳 3–5 条要点",
		f"掌握“{title}”的核心流程/方法/指标，并在示例中正确应用",
	]
...
def gen_summary(title: str, kw: list[str], style: str) -> list[str]:
	...
	profile = STYLE_PROFILES.get(style, STYLE_PROFILES['balanced'])
	return [
		f"本节要点（3–5 条）：围绕“{title}”梳理概念、流程、方法与指标",
		f"关键词：{top}",
		"易错点/反模式：结合团队现状列举 2–3 个常见问题，并给出纠正建议",
		f"方法论/实践：{profile['summary_method']}",
		"实践建议：从小处着手（样例/基线/自动化），建立指标闭环并持续改进",
	]

def gen_exercises(title: str, style: str) -> list[str]:
	profile = STYLE_PROFILES.get(style, STYLE_PROFILES['balanced'])
	return [
		f"场景化练习：结合你的项目，描述“{title}”的一个实践场景，给出输入/步骤/预期输出",
		f"指标设计：为“{title}”设计 3 个可量化指标（含基线/阈值/采样频率），并说明采集方式",
		f"风险与对策：列出 2–3 个与“{title}”相关的风险点，并给出可执行的缓解方案",
		profile['exercise_extra'],
	]
...
def find_subhead_block(lines: list[str], sec_start: int, sec_end: int, name: str):
	# returns (exists, sh_start, sh_end). If not exists: (False, insert_pos, insert_pos)
	pat = re.compile(rf'^#{"".join(["#"]*(SUBHEAD_LEVEL-1))}#\s+{re.escape(name)}\s*$', re.UNICODE)
	...
	# block until next heading level <= SUBHEAD_LEVEL or next same-level subhead
...
def remove_existing_markers(lines: list[str], start: int, end: int) -> list[str]:
...
def fill_book(args):
	...
	# Process in reverse so earlier section indices remain valid after insertions
	for sec_start, sec_end, lvl, title in reversed(secs):
		...
		changed_this_section = False
		# for each subhead
		for name in SUBHEAD_TITLES:
			exists, sh_start, sh_end = find_subhead_block(new_lines, sec_start, sec_end, name)
			if exists:
				# determine if block is placeholder-like; content is between sh_start+1 and sh_end
				content = new_lines[sh_start+1:sh_end]
				already_injected = any(BEGIN_MARK in l for l in content)
				if not rewrite and not is_placeholder_block(content) and not already_injected:
					continue  # keep authored content
				# replace content region with generated
				new_new = []
				if name == '学习目标':
					new_new = [BEGIN_MARK] + [f'- {b}' for b in gen_learning_goals(title, kw, style)] + [END_MARK]
				elif name == '小结':
					new_new = [BEGIN_MARK] + [f'- {b}' for b in gen_summary(title, kw, style)] + [END_MARK]
				else:
					new_new = [BEGIN_MARK] + [f'1. {b}' for b in gen_exercises(title, style)] + [END_MARK]
				new_lines = new_lines[:sh_start+1] + new_new + new_lines[sh_end:]
				updates += 1
				changed_this_section = True
			else:
				# insert a new subhead with content at insert position
				insert_at = sh_start
				block = [f"{'#'*SUBHEAD_LEVEL} {name}"]
				if name == '学习目标':
					block += [BEGIN_MARK] + [f'- {b}' for b in gen_learning_goals(title, kw, style)] + [END_MARK]
				elif name == '小结':
					block += [BEGIN_MARK] + [f'- {b}' for b in gen_summary(title, kw, style)] + [END_MARK]
				else:
					block += [BEGIN_MARK] + [f'1. {b}' for b in gen_exercises(title, style)] + [END_MARK]
				new_lines = new_lines[:insert_at] + block + new_lines[insert_at:]
				updates += 1
				changed_this_section = True
		# refresh section boundaries after modifications
		# No need to recompute section ranges due to reverse order insertion strategy
	...
	rep.write_text(
		f'# Fill learning blocks report\n\nStyle: {style}\nRewrite: {rewrite}\nSections processed: {sec_count}\nBlocks updated/inserted: {updates}\nOutput: {out.name}\nSource: {src.name}\n',
		encoding='utf-8'
	)
	uprint(f'Filled learning blocks -> {out.as_posix()} | style={style} rewrite={rewrite} sections={sec_count} updates={updates} | Report: {rep.name}')


def parse_args(argv=None):
	ap = argparse.ArgumentParser(description='Fill 学习目标/小结/练习 blocks for book sections.')
	ap.add_argument('--input', default=os.environ.get('LEARNING_BLOCK_INPUT', str(DEFAULT_SRC)))
	ap.add_argument('--output', default=os.environ.get('LEARNING_BLOCK_OUTPUT', str(DEFAULT_OUT)))
	ap.add_argument('--style', choices=['academic', 'industry', 'balanced'], default=os.environ.get('LEARNING_BLOCK_STYLE', 'balanced'))
	ap.add_argument('--rewrite', action='store_true', default=os.environ.get('LEARNING_BLOCK_REWRITE','0')=='1')
