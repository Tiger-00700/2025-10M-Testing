"""Generate fuzzy anchor mapping suggestions for unresolved internal links in cleaned book.
...
Strategy:
  ...
  - For each broken target, compare its base (strip numeric suffix) with available anchor bases and heading slug bases using difflib SequenceMatcher ratio.
  - Produce candidates with ratio >= MIN_RATIO (default 0.78).
  - Mark 'confident' if exactly one candidate with ratio >= CONF_RATIO (default 0.90).
...
def main():
    ...
    for tgt in sorted(broken):
        ...
        suggestions.append({
            'target': tgt,
            'base': base,
            'candidates': [{'anchor_base': cand, 'ratio': round(r,3)} for (r,c) in top],
            'confident': confident,
        })
    ...
    md_lines = [f'# Fuzzy Anchor Suggestions CLEANED ({ts})','',f'Broken targets: {len(broken)}',f'Suggestions generated: {len(suggestions)}','']
    for s in suggestions[:400]:
        md_lines.append(f"## {s['target']} → candidates" + (' (CONFIDENT)' if s['confident'] else ''))
