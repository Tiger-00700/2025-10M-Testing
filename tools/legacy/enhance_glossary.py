...
KNOW: Dict[str, str] = {
    "SLA": (
        "服务级别协议：面向业务的可用性与性能承诺，"
        "常以SLO/SLI衡量并落地至告警阈值与运维流程。"
    ),
    "SLI": (
        "服务级别指标：用于度量SLO达成的可观测指标，"
        "如延迟、错误率、吞吐等。"
    ),
    "SLO": (
        "服务级别目标：对SLI的目标阈值设定，例如P95延迟≤200ms、"
        "可用性≥99.9%。"
    ),
    "RPO": (
        "恢复点目标：容灾场景中可接受的数据丢失窗口（时间范围），"
        "决定备份与复制策略。"
    ),
    "RTO": (
        "恢复时间目标：故障发生到恢复服务的时间上限，"
        "影响高可用架构与演练频率。"
    ),
    "ACID": (
        "数据库事务四要素：原子性、一致性、隔离性、持久性；"
        "保证强一致写入与读写隔离。"
    ),
    "BASE": (
        "分布式领域的最终一致性取向：基本可用、软状态、最终一致性，"
        "与ACID形成对照。"
    ),
    "CAP": (
        "一致性/可用性/分区容错性的权衡定理；"
        "实际工程常选CP/AP并通过补偿实现业务一致性。"
    ),
    "Kafka": (
        "分布式日志与消息系统，支持高吞吐顺序写入，"
        "常用于数据采集与流处理管道。"
    ),
    "Flink": (
        "流批一体计算引擎，支持事件时间/状态快照/Exactly-Once，"
        "适配实时计算与CEP场景。"
    ),
    "Spark": (
        "通用大数据计算引擎，RDD/Dataset/Structured Streaming，"
        "批流均可，生态丰富。"
    ),
    "Hadoop": (
        "早期大数据生态核心：HDFS存储+MapReduce计算+YARN资源管理，"
        "现多与湖仓/云原生融合。"
    ),
    "Iceberg": (
        "数据湖表格式：支持ACID、Schema演进、Time Travel、分区进化，"
        "方便批流与多引擎共享。"
    ),
    "Hudi": (
        "数据湖表格式：支持Upsert/CDC与增量拉取，"
        "适合近实时入湖与湖仓一体。"
    ),
    "Delta Lake": (
        "表格式：ACID、版本化与时光回溯，搭配Spark生态广泛应用于湖仓一体。"
    ),
    "Lakehouse": (
        "湖仓一体：统一数据湖与数仓的存储/管理/治理，"
        "支持批流融合与多引擎共享。"
    ),
    "OLAP": (
        "联机分析处理：面向聚合/多维分析与长查询，"
        "强调列存、并行与向量化。"
    ),
    "OLTP": (
        "联机事务处理：面向高并发小事务，强调低延迟强一致与索引优化。"
    ),
    "Kubernetes": (
        "容器编排（K8s）：弹性伸缩、调度与隔离；"
        "大数据计算常在K8s上云原生化部署。"
...
def write_glossary(gloss_lines: List[str], rows: List[List[str]]) -> str:
    ...
    # iterate while the following lines look like table rows starting with '|'
    while (
        tail_start < len(gloss_lines)
        and gloss_lines[tail_start].strip().startswith("|")
    ):
        tail_start += 1
...
def enhance_brief(term: str, current: str) -> str:
    ...
    if term in KNOW:
        return KNOW[term]
    # generic draft
    # Return a concise draft description (split into parts to avoid E501)
    return (
